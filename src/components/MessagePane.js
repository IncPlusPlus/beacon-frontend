  ...
  return (
    <div id="channelPane">
      ...
      <ol id="messagePane" ref={messageList} onScroll={scrollHandler}>
        {props.messages ? (
          Array.from(props.messages.values()).map(
            (msg, index, arr) => (
              <Message
                key={msg.id}
                message={msg}
                minimal={index > 0 && arr[index - 1].senderId === msg.senderId}
              />
            )
      ...
      {messagesBelow ? (
        <span className="messageAlert" onClick={() => scrollToBottom()}>
          More messages below
        </span>
      ) : (
        <></>

  ...
  const handleKeyDown = (e, field) => {
    if (e.key === "Escape") {
      ...
      if (editText !== props.message.messageBody) {
        props.message.editMessage(editText).finally(() => {
          setEditMode(false);
        });
      } else {
    ...
    <div className={"message" + (props.minimal ? " minimal" : "")}>
      {!props.minimal && (
        <img
          ...
          alt={senderUsername}
          src={senderAvatarUrl || default_avatar}
        />
      )}
        ...
        {editMode ? (
          ...
          <>
            <div className="messageContent">
              <ReactMarkdown>{props.message.messageBody}</ReactMarkdown>
            </div>
      ...
      {props.message.senderId === accountId && !editMode && (
        <span className="messageActionContainer">
          ...
          <span className="messageActionButton" onClick={() => enterEditMode()}>
            Edit
          </span>
          <span> | </span>
          <span
            className="messageActionButton"
            onClick={() => props.message.deleteMessage()}
          >
            Delete
          </span>
        </span>

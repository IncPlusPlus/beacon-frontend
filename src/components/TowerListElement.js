...
export const TowerListElement = observer(function TowerListElement(props) {
  ...
  const onMessageEvent = (messageEvent) => {
    switch (messageEvent.type) {
      case "CREATED":
        ...
        tower.channels
          .get(messageEvent.message.channel_id)
          ?.handleMessageCreated(messageEvent.message);
        break;
      case "EDITED":
      ...
      case "DELETED":
        ...
        tower.channels
          .get(messageEvent.message.channel_id)
          ?.handleMessageDeleted(messageEvent.message);
        break;
      default:
        console.log(
          `Received a message event from the server of unknown type '${messageEvent.type}'.`
        );
        break;
    }
  };
  ...
  const onChannelEvent = (channelEvent) => {
    switch (channelEvent.type) {
      ...
      default:
        console.log(
          `Received a channel event from the server of unknown type '${channelEvent.type}'.`
        );
        break;
    }
  };
  ...
  return (
    <>
      ...
      >
        {tower.iconUrl ? (
          <img src={tower.iconUrl} alt={tower.name} />
        ) : (

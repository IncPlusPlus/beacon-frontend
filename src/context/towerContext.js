...
import {
  ChannelsApi,
  Configuration as CityConfiguration,
  InvitesApi,
  UsersApi,
} from "beacon-city";
...
class Towers {
        ...
        if (!this.towers.has(memberTower.id)) {
          this.towers.set(
            ...
            new ObservableTower(
              ...
              city.id,
              memberTower.name,
              memberTower.adminAccountId,
              memberTower.moderatorAccountIds,
              memberTower.memberAccountIds
            )
          );
        }
      }
  ...
  *generateInviteCode(towerId, expiryTime, expiryTimeUnit, maxUses) {
    return yield new InvitesApi(this.cityConfig(towerId))
      .createInvite({
        towerId: towerId,
  ...
  *joinTower(code) {
    const context = this;
    yield new CisInvitesApi(this.cisConfig)
      .getInviteInfo({
        towerInviteCode: code,
      })
      .then((invite) => {
        // Retrieve the city base path
        new CityManagementApi(this.cisConfig)
          .getCity({
          ...
          .then((city) => {
            ...
            new InvitesApi({
              ...
              .joinUsingInvite({ towerInviteCode: code })
              .then(() => {
  ...
  *createChannel(towerId, newChannelName) {
    yield new ChannelsApi(this.cityConfig(towerId))
      .createChannel({
        towerId: towerId,

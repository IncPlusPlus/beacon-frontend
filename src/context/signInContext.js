...
export const SignInContext = createContext({
  ...
  attemptSignIn: (username, password) =>
    console.log(
      "WARNING: Attempted to sign in but hit a no-op function. This means that the SignInContext was never set up properly!!!"
    ),

  signupState: "",
  attemptSignUp: (username, email, password) =>
    console.log(
      "WARNING: Attempted to sign up but hit a no-op function. This means that the SignInContext was never set up properly!!!"
    ),
  /**
  ...
  invalidateSession: () =>
    console.log(
      "WARNING: Attempted to invalidate session but hit a no-op function. This means that the SignInContext was never set up properly!!!"
    ),
});
...
export function SignInContextProvider({ children }) {
  ...
  const [currentPassword, setCurrentPassword] = useLocalStorage(
    "currentPassword",
    ""
  );
...
function signUpWithCredentials(
  ...
  new AccountManagementApi(configuration)
    .createNewAccount({
...
function signInWithCredentials(
  ...
  if (signInInProgress) {
    console.log(
      "Attempted to sign in while there was already another attempt in progress. This attempt has been cancelled."
    );

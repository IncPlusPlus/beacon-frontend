...
export class ObservableChannel {
  ...
  *fetchMessages() {
    console.log(
      `Refreshing messages for tower ${this.towerId}, channel ${this.id}.`
    );
    ...
    try {
      const response = yield new MessagesApi(
        this.cityConfig(this.towerId)
      ).getMessages({
      ...
      console.log(
        `Updated messages for tower ${this.towerId}, and channel ${this.id}`
      );
    } catch (error) {
  ...
  handleMessageCreated(message) {
    console.log(
      `Received new message for tower ${this.towerId}, channel ${this.id}`
    );
  ...
  handleMessageEdited(message) {
    console.log(
      `Message ${message.id} for tower ${this.towerId}, channel ${this.id} has been edited`
    );
    // Field names use snake_case for some reason when we receive objects through STOMP/Websockets
  ...
  *sendMessage(messageBody) {
    console.log(
      `Sending message in tower ${this.towerId}, channel ${this.id}.`
    );
    ...
    try {
      ...
      yield new MessagesApi(this.cityConfig(this.towerId)).createMessage(
        request
      );
    } catch (error) {
      // TODO: Make this a snackbar with notistack

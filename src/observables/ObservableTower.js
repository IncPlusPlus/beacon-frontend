...
export class ObservableTower {
  ...
  *initializeChannels() {
    ...
    if (this.channels.size === 0) {
      ...
      try {
        const channels = yield new ChannelsApi(
          this.cityConfig(this.id)
        ).getChannels({ towerId: this.id });
        channels.forEach((channel) => {
          this.channels.set(
            channel.id,
            new ObservableChannel(
              this.cityConfig,
              channel.id,
              channel.towerId,
              channel.name,
  ...
  handleChannelCreated(channel) {
    console.log(
      `New channel ${channel.id} has been created in tower ${this.id}`
    );
